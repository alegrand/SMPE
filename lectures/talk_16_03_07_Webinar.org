...
- Slide "My feeling" is irrelevant at that point
- But do we really have to care in CS ? should be extended to other
  use cases (E.g., graphics, data mining ?)
- Trying to Bridge the Gap: should start wit only the Published article
- After the Sumatra example: add Jupyter screenshot, possibly a
  CARE/CDE example, and exec&share.
- Difficult Trade-off: Automatic vs. Explicit
- 6. Controlling your experiment: 
  - Mention workflow
  - Mention numerical reproducibility
- Link "reproducible research actions" with "following key points"
- Talk about ACM branding for artifacts ?
...
#+BEGIN_CENTER
\bf We intend to start on March 7, 2016
#+END_CENTER
