...
#+begin_src R :results output graphics :file pdf_babel/tcl_large_numbers.pdf :exports results :width 7 :height 3 :session
library(ggplot2)
N=10000;
...
$$\P(|\rv{S_n}/n-\mu|\geq \epsilon) \leq \frac{\sigma^2}{n\epsilon^2} = \frac{100}{n} = 0.5 \quad\frowny$$
...
- For large $n$'s, the distribution of $\rv{S^*_n}$ is approximately
  *normal*
  #+BEGIN_EXPORT latex
  \begin{equation*}
...
\begin{align*}
  \M & = \int e^{tx}f_\N(x).dx
       ...
       = \int \frac{e^{\frac{1}{2}(-x^2+tx)}}{\sqrt{2\pi}}dx \\
...
\begin{align*}
  \M[\rv{S^*_n}] & = \E(e^{t\rv{S^*_n}}) 
                   = \E(e^{t{\frac{\rv{S_n}-n\mu}{\sqrt{n}\sigma}}}) 
