...
*** Replicable article \qquad (Literate programming)
\small
...
*** Logging your activity \qquad (Laboratory Notebook)
...
- The cloud! (Dropbox $\frowny$, Google Drive, ...)
- Flexible version control systems (e.g. git $\smiley$) where you're in
  control of what's happening
  - Can be in a crontab if you really do not want to think about it
...
- *Use R*, Python or another free software to read and process raw
  ...
  - The org tangling mechanism may help
*** Capturing your environment
What are the options?
- Nothing \winkey
- Restrict your tools/dependencies to the bare minimum and list them
  all. This may not be sufficient but it will help.
- Have custom shell scripts that log all the dependencies you are
  aware of and ask your friends to check whether this information is
  sufficient...
- Restrict your tools/dependencies and combine everythin in an
  [[http://www.activepapers.org/][/activepapers/]], i.e., an HDFS5 file combining datasets and programs
  working on these datasets in a single package, along with meta data,
  history, ...
- Have tools that *automatically* keep track of dependencies/files
  and packages up the Code, Data, and Environment 
  - [[http://www.pgbovine.net/cde.html][CDE]] (Guo et al., 2011) [[https://vida-nyu.github.io/reprozip/][ReproZip]] (Freire et al., 2013) or more
    recently [[http://reproducible.io/][CARE]] (Janin et al., 2014)
- Create and distribute your own virtual image (VM, docker,
